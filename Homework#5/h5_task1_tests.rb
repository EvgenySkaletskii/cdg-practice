...
RSpec.describe "Test CashMachine methods" do
  BALANCE_FILE = "balance.txt"
  subject { CashMachine.new }

  before do
    File.write(BALANCE_FILE, "200.0")
    subject.read_balance
  end

  it "Should read balance from the file" do
    expect(subject.balance).to eq(200.0)
  end
  ...
  it "Should show balance" do
    expect { subject.show_balance }.to output("Ваш баланс: 200.0\n").to_stdout
  end

  it "Should puts money on deposit" do
    expect { subject.deposit(100) }.to output("Ваш баланс: 300.0\n").to_stdout
    expect(subject.balance).to eq(300.0)
  end

  it "Should return error when deposit amount less than 0" do
    expect { subject.deposit(-100) }.to output("Сумма не может быть меньше или равна нулю.\n").to_stdout
  end

  it "Should return error when deposit amount equal to 0" do
    expect { subject.deposit(0) }.to output("Сумма не может быть меньше или равна нулю.\n").to_stdout
  end

  it "Should withdraw money from balance" do
    expect { subject.withdraw(100) }.to output("Ваш баланс: 100.0\n").to_stdout
  end

  it "Should withdraw all money from balance" do
    expect { subject.withdraw(200) }.to output("Ваш баланс: 0.0\n").to_stdout
  end

  it "Should return error when value more than balance" do
    expect { subject.withdraw(1000) }.to output("Недостаточно средств на счету. Ваш баланс 200.0\n").to_stdout
  end

  it "Should return error when withdraw value less than 0" do
    expect { subject.withdraw(-50) }.to output("Сумма не может быть меньше или равна нулю\n").to_stdout
  end

  it "Should return error when withdraw value equal to 0" do
    expect { subject.withdraw(0) }.to output("Сумма не может быть меньше или равна нулю\n").to_stdout
  end

  it "Should record balance into the file on quit" do
    subject.deposit(300)
    subject.record_balance
    expect(File.read(BALANCE_FILE)).to eq("500.0")
  end
end

RSpec.describe "Test Cashmachine init method" do
  ...
  it "Should run terminal commands" do
    ...
    expect { CashMachine.init }.to output(result).to_stdout
  end
